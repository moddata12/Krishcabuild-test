...
function Report() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:3001/api/Report4Data");
  ...
  const handleOkClick = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3001/api/report4Date?start_date=${fromDate}&end_date=${toDate}`
      );
  ...
  const handleExportClick = async () => {
    try {
      ...
      const response = await fetch("http://localhost:3001/api/exportReport4");
