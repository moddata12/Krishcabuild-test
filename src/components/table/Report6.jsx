...
function Report() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:3001/api/Report6Data");
  ...
  const handleOkClick = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3001/api/report6Date?start_date=${fromDate}&end_date=${toDate}`
      );
  ...
  const handleExportClick = async () => {
    try {
      ...
      const response = await fetch("http://localhost:3001/api/exportReport6");
