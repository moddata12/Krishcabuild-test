...
function Report() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:3001/api/Report13Data");
  ...
  const handleOkClick = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3001/api/report13Date?start_date=${fromDate}&end_date=${toDate}`
      );
  ...
  const handleExportClick = async () => {
    try {
      ...
      const response = await fetch("http://localhost:3001/api/exportReport13");
