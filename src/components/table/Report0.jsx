...
function Report() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:3001/api/Report0Data");
  ...
  const handleOkClick = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3001/api/report0Date?start_date=${fromDate}&end_date=${toDate}`
      );
  ...
  const handleExportClick = async () => {
    try {
      ...
      const response = await fetch("http://localhost:3001/api/exportReport0");
